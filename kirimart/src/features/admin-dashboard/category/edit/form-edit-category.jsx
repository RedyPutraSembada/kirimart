...
export function FormEditCategory({ dataCategory }) {
	...
	const [imagePreview, setImagePreview] = useState(dataCategory?.iconUrl || null)
	...
	const form = useForm({
		...
		defaultValues: {
			...
			isActive: dataCategory?.isActive ?? true,
		},
	})
	...
	const updateMutation = useMutation({
		mutationFn: (data) => updateCategory(dataCategory.id, data),
		onSuccess: () => {
		...
		<div className="space-y-6 max-w-3xl mx-auto">
			<div className="flex items-center gap-4">
				...
				<div>
					...
					<p className="text-muted-foreground">Ubah informasi master data kategori.</p>
				</div>
			</div>
									...
									<FormItem>
										...
										>
											...
											<SelectContent>
												...
												{categoriesDropdown?.data?.filter(c => c.id !== dataCategory.id).map((c) => (
									...
									<FormItem>
										...
										<FormControl>
											<Input
												...
												onChange={(e) => {
													field.onChange(e)
													// Auto-generate slug if it's identical to the generated one before
